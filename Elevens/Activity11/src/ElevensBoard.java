...
public class ElevensBoard extends Board {
		...
		for (int sk1 = 0; sk1 < selectedCards.size(); sk1++) {
			...
			for (int sk2 = sk1 + 1; sk2 < selectedCards.size(); sk2++) {
				...
				if (cardAt(k1).pointValue() + cardAt(k2).pointValue() == 11) {
					List<Integer> pairSum11 = new ArrayList<>();
                    pairSum11.add(k1);
                    pairSum11.add(k2);
                    return pairSum11;
				}
	...
	private List<Integer> findJQK(List<Integer> selectedCards) {
        ...
        for(int cIndex: selectedCards) {
            int pointValue = cardAt(cIndex).pointValue();
            if(pointValue > 10)
                jqkIndexes.set(pointValue-11, cIndex);
        }

        return (jqkIndexes.contains(-1)) ? null : jqkIndexes;
	}
	...
	private boolean playPairSum11IfPossible() {
        List<Integer> cardsToRemove = findPairSum11(cardIndexes());
        if(cardsToRemove != null) {
            replaceSelectedCards(cardsToRemove);
            return true;
        }
        return false;
	}
	...
	private boolean playJQKIfPossible() {
        List<Integer> cardsToRemove = findJQK(cardIndexes());
        if(cardsToRemove != null) {
            replaceSelectedCards(cardsToRemove);
            return true;
        }
